...
$classes = Flux::classes()
    ...
    ->add($invalid
        ...
        : 'border border-neutral-200 border-b-neutral-300/80 dark:border-white/10'
    )
    ;
...
<div {{ $attributes->class($classes) }} data-flux-file-item>
    <div class="{{ $figureWrapperClasses }}">
        <flux:icon name="{{ $icon }}" variant="{{ $iconVariant }}" class="text-neutral-400 [&:has(+[data-slot=image])]:hidden" />
    ...
    <div class="flex-1 overflow-hidden py-[calc(0.75rem-3px)] me-3 flex flex-col justify-center gap-1" data-slot="content">
        <?php if ($heading): ?>
            <div class="text-sm font-medium text-neutral-500 dark:text-white/80 whitespace-nowrap overflow-hidden text-ellipsis">{{ $heading }}</div>
        <?php endif; ?>

        <?php if ($text): ?>
            <div class="text-xs text-neutral-500">{{ $text }}</div>
        <?php endif; ?>
    </div>
